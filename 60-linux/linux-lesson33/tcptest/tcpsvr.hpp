...
class TcpServer
{
    public:
        ...
        bool Send(const struct CliInfo& cli_info)
        {
            int sendsize = send(Sock_, &cli_info, sizeof(cli_info), 0);
            if(sendsize < 0)
            {
                perror("send");
                return false;
        ...
        bool Recv(std::string* Name, std::string* Address)
        {
            //flags
            //  0 阻塞接收
            //  MSG_PEEK 探测接收，由于TCP套接字是一个流式套接字，所以有可能数据接收的时候出现数据不完整的情况，对于上次协议所定义的数据不完整
            struct CliInfo cli_info;
            int recvsize = recv(Sock_, &cli_info, sizeof(cli_info), 0);
            if(recvsize < 0)
            {
                perror("recv");
                return false;
            ...
            (*Name).assign(cli_info.Name_, 20);
            (*Address).assign(cli_info.Address_, 20);
            return true;
        }
