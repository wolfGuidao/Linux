...
int main()
{
  ...
  server.Post("/dish",[&dish_table](const Request& req,Response& resp){
      ...
      printf("新增菜品！\n");
      //1.获取到数据并解析成json格式
      bool ret = reader->parse(req.body.c_str(),req.body.c_str()+req.body.length(),&req_json,&err); 
      if(!ret)
      {
      //如果失败构造响应的响应给客户端
      ...
      resp.status=400;
      writer->write(resp_json,&os);
      resp.set_content(os.str(),CONTENT_TYPE);
      return;
      }
  ...
  server.Get("/dish",[&dish_table](const Request& req,Response& resp){
      ...
      resp.status=500;
      writer->write(resp_json,&os);
      resp.set_content(os.str().c_str(),CONTENT_TYPE);
      return ;
  ...
  server.Get(R"(/dish/(\d+))",[&dish_table](const Request& req,Response& resp){
      ...
      resp.status=500;
      writer->write(resp_json,&os);
      resp.set_content(resp_json.asCString(),CONTENT_TYPE);
      return ;
  ...
  server.Post("/order",[&order_table](const Request& req,Response& resp){
      ...
      req_json["dishes"]=writer->write(dish_ids,&os);
